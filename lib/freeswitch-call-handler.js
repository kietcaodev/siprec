...
const handler = (req, res) => {
  ...
  logger.info(`received SIPREC invite: ${req.source_address} ${req.uri}`);
...
/**
Kiet custom with Version 17.06.05
 */	  
console.log(opts.callee.aor);
// Regular expression to match the IP address
var ipAddressRegex = /(?:\d{1,3}\.){3}\d{1,3}/;
// Extracting the IP address from the input string
var ipAddress = opts.callee.aor.match(ipAddressRegex);
// If a match is found, ipAddress will contain the IP address
if (ipAddress !== null) {
    console.log("IP Address:", ipAddress[0]);
} else {
    console.log("No IP address found in the input string.");
}
      const headers = {
        'X-Return-Token': opts.sessionId,
		'X-Destination-ID': req.uri,
        'X-SBC-Call-ID': opts.req.get('Call-ID'),
		'X-SBC-aor': opts.caller.aor,
		'X-SIP-IP': ipAddress ? ipAddress[0] : ''
      };
...
function setDialogHandlers(logger, {uas, uac, req, res, opts}) {
  uas
    .on('destroy', () => {
    ...
    .on('modify', (req, res) => {
	logger.info(`received re-INVITE from SBC ${req.get('User-Agent')} ${req.source_address} ${req.uri} `);
      res.send(200, {
        body: uas.local.sdp
      });
    });
