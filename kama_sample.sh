...
freeswitch.bindip = "FreeSWITCH.IP" desc "FreeSWITCH IP Address"
...
route[FSDISPATCH] {
	...
	switch($rU) {
		case /"^441[0-9][0-9][0-9]$":
			xlog("L_INFO", "------ Debug prefix 44 ------\n");
		...
		default:
			...
			{
				xlog("L_INFO", "------ Debug no-register ------\n");
				route(FSRELAY);
				exit;
			}
			# online - do bridging
			xlog("L_INFO", "------ Debug register ------\n");
			prefix("kb-");
			if(is_method("INVITE"))
			{
...
route[DISPATCH] {
	...
	xlog("L_INFO", "------ SCRIPT: going to $fd --> $rd --> $dd \n");
	xlog("L_INFO", "------ SCRIPT: Route to dispatcher\n");
...
failure_route[RTF_DISPATCH] {
	...
	# Check status 486(Busy)
		if (t_check_status("486")) {
		xlog("L_INFO", "------ Status call Busy\n");
		$du = $null;
		exit;
		}
	...
	if (t_check_status("500") or (t_branch_timeout() and !t_branch_replied())) {
		if(ds_next_dst()) {
			xlog("L_INFO", "------ SCRIPT: retrying to $fd --> $rd --> $dd\n");
			t_on_failure("RTF_DISPATCH");
			route(RELAY);
			exit;
		}
